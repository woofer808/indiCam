...
if (indiCam_var_actionValue == 0) then { // Low action value scenes
	...
	indiCam_var_scene = selectRandom [ // Choose a random scene from the list
								...
								//"droneCamPosition",		// Angled top-down view that rotates around a fixed position
...
if (indiCam_var_actionValue == 1) then { // Medium action value scenes
	...
	switch (indiCam_var_scene) do {
		...
		case "followClose": {
			...
			if ( (stance actor) == ("UNDEFINED") ) then {_posZ = 20};
			if ( (stance actor) == ("") ) then {_posZ = 10};
			if ( (stance actor) == ("PRONE") ) then {_posZ = 0.5};			
			if ( (stance actor) == ("CROUCH") ) then {_posZ = selectRandom [1,0.5]};
			if ( (stance actor) == ("STAND") ) then {_posZ = selectRandom [1.5,1,0.5]};
