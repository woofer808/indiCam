...
comment "																										";
comment "	This script manages who the actor is at a given time and assigns/removes properties as needed.		";
comment "																										";
comment "-------------------------------------------------------------------------------------------------------";


// This function removes eventhandlers stored in the global variable
...
/*	params [
	...
	*/
	...
	if (_restoreActor) then {
		...
		actor removeEventHandler ["Killed",indiCam_var_actorFiredEH];
		actor removeEventHandler ["Deleted",indiCam_var_actorFiredEH];
		...
		// Detect actor dying
		indiCam_var_actorFiredEH = _newActor addEventHandler ["Killed", {
			indiCam_var_scriptedActor = actor;
			indiCam_var_scriptedScene = true;
			indiCam_var_scriptedSceneType = "actorDeath";
			indiCam_var_interruptScene = true;
			if (indiCam_debug && indiCam_runIndiCam) then {systemChat "actor was killed"};
		}];
		...
		indiCam_var_actorFiredEH = _newActor addEventHandler ["Deleted", {
			// This is where we stop all eventhandlers
			// Kill any onEachFrame camera eventhandlers still going.
			[] call indiCam_fnc_clearEventhandlers;
			if (indiCam_debug && indiCam_runIndiCam) then {systemchat "actor was deleted";};
		}];
	...
	// Just return the new actor for simplicity in case we need that
	_newActor;
...
// Spawn the auto switching

indiCam_fnc_actorAutoSwitchActive = {

	while {indiCam_var_actorAutoSwitchCheckboxState} do {
		...
		private _changeCheck = indiCam_var_actorAutoSwitchDuration;
		while {time <= _future} do {
			//if (!indiCam_runIndiCam) exitWith {indiCam_var_actorAutoSwitch = false;}; // not needed now?
			if (!indiCam_var_actorAutoSwitch) exitWith {}; // Stops the loop
			
			if (
					!(indiCam_var_actorAutoSwitchDuration isEqualTo _changeCheck)
					&&
					((time - indiCam_var_guiActorAutoswitchSliderChanged) > 3 ) 
				) exitWith {if (indiCam_debug) then {systemchat "updated actor auto switch time"}}; // Stops the loop
