...
[] spawn { // Setup the logics for camera point and the camera target point. Hide them or use invisible if debug is turned off.
	...
	if (indiCam_debug) then { // When debug is on, create all logics
	...
	} else { // When debug is off, create all logics and hide them
...
while {indiCam_runIndiCam} do {
	...
	if (!alive actor) then {
		...
		[] call indiCam_fnc_actorSwitchToClosest;
	};


	// Reset variable for any scripted scenes that might have been run
	// Should probably be done directly in exitWith down below, but apparently I don't know how to use exitWith
...
comment "-------------------------------------------------------------------------------------------------------";
	...
	while {time <= _future} do {
		...
		if (indiCam_var_scriptedSceneDone) exitWith {};
