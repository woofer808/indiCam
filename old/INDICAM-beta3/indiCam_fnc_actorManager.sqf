...
indiCam_fnc_mapSelectAll = {
	...
	// Force open map and have the user click a point close to a target
	...
	onMapSingleClick {
		_mapClickPos = _pos;
		private _objectArray = nearestObjects [_mapClickPos,["Man","Car","Tank","Helicopter","Plane","Ship"],100,true]; // Vehicles probably aren't needed here, it seems I can make those actors as well which I don't want
		private _unitArray = [];
		...
		if (count _objectArray > 0) then {
			{
				...
				if (
					...
					&& (alive _unit)
					&& ((_unit isKindOf "Man") || (_unit isKindOf "Car") || (_unit isKindOf "Tank") || (_unit isKindOf "Helicopter") || (_unit isKindOf "Plane") || (_unit isKindOf "Ship"))// Vehicles probably aren't needed here, it seems I can make those actors as well which I don't want
					) then {
