...
indiCam_fnc_EHGetInMan = {
	...
	if (indiCam_debug && (indiCam_var_currentMode == "default")) then {
		systemChat "actor mounted a vic"
	};
};
...
indiCam_fnc_EHGetOutMan = {
	indiCam_var_requestMode = "default";
	if (indiCam_debug && (indiCam_var_currentMode == "default")) then {
		systemChat "actor dismounted a vic"
	};
};
...
indiCam_fnc_EHFired = {
	...
	if (indiCam_debug && (indiCam_var_currentMode == "default")) then {
		systemChat format ["actor has fired at time %1", time]
	};
};

indiCam_fnc_EHDeleted = {
	// This is where we stop all eventhandlers by switching unit
	if (indiCam_debug && indiCam_running) then {
		systemchat "actor was deleted";
	};
...
indiCam_fnc_EHKilled = {
	if (indiCam_debug && indiCam_running) then {
		systemChat "indiCam_actor was killed"
	};
