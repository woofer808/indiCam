...
if (indiCam_var_visionIndex == 0) then {
	...
	// At 04:30, turn on daylight (based on Altis)
	if ( (_time >= 430) && (_time <= 1930) ) then {
...
comment "-------------------------------------------------------------------------------------------------------";
	...
	if (indiCam_var_scriptedScene) then {_vicValue = 6}; // If an something like an AT scene is in effect, run this
