...
_currentActorDisplay ctrlSetText format ['Current actor: %1', (name actor)];
...
// A script that stops any running background functions when the camera is stopped.
indiCam_fnc_guiStop = {
	
	if (indiCam_debug) then {systemChat "Stopping all functions through GUI";};
...
_sliderMaxAutoSwitch = 300;

_controlSliderAutoSwitch = (findDisplay indiCam_id_guiDialogMain) displayCtrl indiCam_id_guiSceneOverrideSlider; // Define the displaycontrol
_controlSliderAutoSwitch sliderSetRange [_sliderMinAutoSwitch, _sliderMaxAutoSwitch];
_controlSliderAutoSwitch sliderSetPosition indiCam_var_guiSceneOverrideSliderState;
...
indiCam_fnc_guiSceneOverride = {
	...
	if (_checkedOverride) then {
		...
		indiCam_var_SceneOverrideState = true;
	} else {
		// If not checked, just store the value in a global var for until it is needed.
...
indiCam_fnc_guiVisionMode = {
	...
	indiCam_var_visionIndex = _index;
...
indiCam_fnc_actorAutoSwitchTimeSlider = {
	...
	indiCam_var_actorAutoSwitchDuration = _sliderPos;
...
indiCam_fnc_guiCheckboxActorAutoswitch = {
	...
	if (_checkedAutoSwitchCheckbox) then {
		...
		indiCam_var_actorAutoSwitch = true;
...
indiCam_fnc_guiDropdownActorAutoswitch = {
	...
	indiCam_var_actorAutoSwitchMode = _index;
...
// This is called from the "set as actor" button at the player listbox
indiCam_fnc_guiPlayerListButton = {
	// Set the selected player as the actor
	[indiCam_var_guiPlayerListSelectedPlayer] call indiCam_fnc_actorChange;
...
_checkArray = actor in indiCam_var_guiPlayerListArray;
if (_checkArray) then {
		_index = indiCam_var_guiPlayerListArray find actor;
...
indiCam_fnc_guiManualMode = {
	...
	// This will start the camera if it isn't already started.
	...
	indiCam camSetPos [(getPos actor select 0)+1,(getPos actor select 1)+1,(getPos actor select 2)+2];
	indiCam camSetFov 0.5;
	indiCam camCommit 0;
	...
	waitUntil {!(indiCam_runIndiCam) or (dialog)};
